...
function Cart() {
  ...
  const buyNow = async () => {
    ...
    var options = {
      key: "rzp_test_ieIBW8MWwCCQlP",
      key_secret: "mM2mmFnQfmkh0eatHKHaCGcG",
  ...
  const handleQuantityChange = (item, quantity) => {
    item.quantity = quantity; // Update item's quantity in cart
    // Trigger re-render by updating state (cartItems)
        ...
        <div className="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0 ">
                ...
                <div className="justify-between mb-6 rounded-lg border  drop-shadow-xl bg-white p-6  sm:flex  sm:justify-start" style={{ backgroundColor: mode === 'dark' ? 'rgb(32 33 34)' : '', color: mode === 'dark' ? 'white' : '', }}>
                  ...
                  <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                    <div className="mt-5 sm:mt-0">
                      ...
                      <p className="mt-1 text-xs font-semibold text-gray-700" onClick={()=>{
                         p = 1;
                         newAmount=prompt('enter the amount')


                        print(newAmount)
                      }} style={{ color: mode === 'dark' ? 'white' : '' }}>₹{p!=0? newAmount: price}</p>
                    </div>
          ...
          <div className="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3" style={{ backgroundColor: mode === 'dark' ? 'rgb(32 33 34)' : '', color: mode === 'dark' ? 'white' : '', }}>
            <div className="mb-2 flex justify-between">
              ...
              }} className="text-gray-700" style={{ color: mode === 'dark' ? 'white' : '' }}>₹{totalAmout}</p>
            </div>
